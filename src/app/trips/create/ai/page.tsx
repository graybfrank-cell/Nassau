        ...
        <div className="mx-auto max-w-3xl">
          ...
          {breakdown && totalCost > 0 && (
            <div className="mt-8 rounded-2xl border border-zinc-200 bg-white p-6">
              ...
              <div className="mt-4 space-y-3">
                {[
                  { label: "Golf", value: breakdown.golf, color: "bg-emerald-500" },
                      ...
                      {day.items?.map((item, i) => (
                        <div key={i} className="flex items-center justify-between text-xs">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={item.type === "tee_time" ? "font-medium text-emerald-600" : "text-zinc-600"}>
                              {item.title}
                            </span>
                          </div>
          ...
          <div className="mt-8 flex gap-3">
            ...
            <button
              onClick={chooseTrip}
              disabled={creating}
              className="flex-1 rounded-lg bg-emerald-600 py-3 text-sm font-bold text-white shadow-lg hover:bg-emerald-700 disabled:opacity-50"
            >
              {creating ? (
        ...
        <div className="mx-auto max-w-2xl">
          ...
          {introMessage && (
            <div className="mt-6 rounded-2xl bg-emerald-50 border border-emerald-100 p-5">
              <p className="text-sm text-emerald-800">{introMessage}</p>
            </div>
          )}
                    ...
                    return (
                      <button
                        ...
                        className={`w-full rounded-xl border-2 p-4 text-left transition-all ${
                          selected ? "border-emerald-500 bg-emerald-50" : "border-zinc-200 hover:border-emerald-300"
                        }`}
                      >
                        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="mt-8 w-full rounded-xl bg-emerald-600 py-3.5 text-base font-bold text-white shadow-lg hover:bg-emerald-700 disabled:opacity-50"
          >
            Build My Trip &rarr;
          </button>
        </div>
        ...
        <div className="mx-auto max-w-4xl">
              ...
              return (
                <div
                  ...
                  className={`rounded-2xl border-2 bg-white shadow-sm overflow-hidden transition-all ${
                    isSelected ? "border-emerald-500 ring-2 ring-emerald-200" : "border-zinc-200"
                  }`}
                >
                  <div className="p-5">
                    {/* Badge */}
                        ...
                        <ul className="mt-1.5 space-y-1">
                          {concept.highlights.map((h, i) => (
                            <li key={i} className="flex items-start gap-2 text-xs text-zinc-600">
                              <span className="mt-0.5 text-emerald-500">&bull;</span> {h}
                            </li>
                          ))}
                        </ul>
                      </div>
                    ...
                    <button
                      ...
                      className={`w-full rounded-lg py-2 text-sm font-semibold transition-colors ${
                        isSelected
                          ...
                          : "border border-zinc-300 bg-white text-zinc-700 hover:border-emerald-300 hover:text-emerald-600"
                      }`}
                    >
                      {isSelected ? (
          ...
          <div className="mt-8 text-center">
            <button
              ...
              className="rounded-xl bg-emerald-600 px-8 py-3.5 text-base font-bold text-white shadow-lg transition-all hover:bg-emerald-700 disabled:opacity-50"
            >
              Tell Me More &rarr;
            </button>
        ...
        <div className="mt-6">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                {VIBES.map((v) => (
                  <button key={v.id} onClick={() => selectVibe(v.id)}
                    className={`rounded-xl border-2 p-4 text-left transition-all hover:border-emerald-300 hover:shadow-md ${vibe === v.id ? "border-emerald-500 bg-emerald-50" : "border-zinc-200 bg-white"}`}>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {GROUP_SIZES.map((n) => (
                  <button key={n} onClick={() => selectGroupSize(n)}
                    className={`rounded-full px-6 py-3 text-sm font-semibold transition-all hover:border-emerald-300 hover:shadow-md ${groupSize === n ? "border-2 border-emerald-500 bg-emerald-50 text-emerald-700" : "border-2 border-zinc-200 bg-white text-zinc-700"}`}>
                    {n}
                  </button>
                ))}
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                {BUDGETS.map((b) => (
                  <button key={b.id} onClick={() => selectBudget(b.id)}
                    className={`rounded-xl border-2 p-4 text-left transition-all hover:border-emerald-300 hover:shadow-md ${budgetTier === b.id ? "border-emerald-500 bg-emerald-50" : "border-zinc-200 bg-white"}`}>
          ...
          {questionStep === 4 && (
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {SEASONS.map((s) => (
                  <button key={s.id} onClick={() => selectSeason(s.id)}
                    className={`rounded-full px-4 py-2 text-sm font-medium transition-all ${season === s.id ? "border-2 border-emerald-500 bg-emerald-50 text-emerald-700" : "border-2 border-zinc-200 bg-white text-zinc-600 hover:border-emerald-300"}`}>
                    {s.label}
                  </button>
                ))}
              </div>
              ...
              {startDate && endDate && (
                <button onClick={confirmDates} className="mt-4 w-full rounded-lg bg-emerald-600 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700">Continue with these dates</button>
              )}
                ...
                {PRIORITIES.map((p) => {
                  ...
                  return (
                    <button key={p.id} onClick={() => togglePriority(p.id)} disabled={maxed}
                      className={`rounded-full px-4 py-2 text-sm font-medium transition-all ${selected ? "border-2 border-emerald-500 bg-emerald-50 text-emerald-700" : maxed ? "border-2 border-zinc-100 bg-zinc-50 text-zinc-300 cursor-not-allowed" : "border-2 border-zinc-200 bg-white text-zinc-600 hover:border-emerald-300"}`}>
                      {p.emoji} {p.id}
                    </button>
                  );
                })}
              </div>
              <button onClick={() => setQuestionStep(6)} disabled={priorities.length === 0} className="mt-6 w-full rounded-lg bg-emerald-600 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-50">Continue</button>
          ...
          {questionStep === 6 && (
            <div>
              ...
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
                ...
                rows={4} className="mt-3 w-full rounded-lg border border-zinc-300 px-4 py-3 text-sm placeholder:text-zinc-400 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" />
              <button onClick={generateConcepts} className="mt-6 w-full rounded-xl bg-emerald-600 py-3.5 text-base font-bold text-white shadow-lg transition-all hover:bg-emerald-700 hover:shadow-xl">
                Find My Trip &rarr;
              </button>
...
function AnsweredBubble({ label, value, onEdit }: { label: string; value: string; onEdit: () => void }) {
  return (
    <div className="mb-3 flex items-start gap-3">
      ...
      <button onClick={onEdit} className="text-xs text-emerald-600 hover:text-emerald-700">Edit</button>
    </div>
  );
}
