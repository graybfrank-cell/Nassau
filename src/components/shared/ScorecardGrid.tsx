...
function getScoreColor(score: number, par: number): string {
  ...
  if (diff === -1) return "bg-emerald-100 text-[#2D5A3D]"; // birdie
                  ...
                  <td key={idx} className="px-0.5 py-0.5">
                    {readOnly && !canEditAll ? (
                      ...
                      <input
                        ...
                        className={`w-full h-8 rounded border border-zinc-200 px-0.5 text-center text-xs focus:border-[#2D5A3D] focus:outline-none focus:ring-1 focus:ring-[#2D5A3D]/20 ${getScoreColor(scores[idx], defaultPars[idx])}`}
                        placeholder="-"
                      />
                    )}
                  </td>
